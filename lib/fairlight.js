...
export const SECTIONS = ['levels', 'eq', 'dynamics', 'inputConfig']
...
/**
 * Fairlight input ids are not video input ids past the HDMI/SDI range: 1001/1002 are the mic
 * inputs and 2001/2002 are the media players. Only embedded-with-video strips may borrow the
 * video input's name — otherwise the mixer ends up showing a mic strip called "Color 1".
 */
function labelFor(state, inputId, inputType) {
	...
	if (inputType === 2) return inputId > 1000 && inputId < 1100 ? `Mic ${inputId - 1000}` : `Audio in ${inputId}`
...
export async function applyChannel(atem, dest, payload, sections) {
	...
	if (sections.levels) {
		...
		if (num(l.faderGain)) cfsp.faderGain = l.faderGain
		if (num(l.balance)) cfsp.balance = l.balance
		...
		if (num(l.stereoSimulation)) {
			...
			else skipped.push('stereoSimulation (destination has none)')
		}
		// mixOption (the channel's on / off / audio-follow-video state) is deliberately never
		// copied. It is a live operating control, not part of a channel's sound — pasting a
		// preset must not mute a channel or put it on air.
	}
...
export function diffChannel(current, incoming, sections) {
	...
	if (sections.levels) {
		...
		for (const k of ['gain', 'faderGain', 'balance', 'framesDelay', 'stereoSimulation']) {
			push('levels', k, current?.levels?.[k], incoming?.levels?.[k])
		}
	}
	if (sections.eq) {
